...
function renderSparklineCell(params: GridCellParams<SparkLineData, unknown>) {
  ...
  const { value, colDef } = params as GridCellParams<SparkLineData, unknown>;

  if (!value || (value as SparkLineData).length === 0) {
    return null;
  }
...
export function renderAvatar(
  ...
  return (
    <Avatar
      sx={{
        backgroundColor: (params.value as { name: string; color: string }).color,
        width: '24px',
        height: '24px',
        fontSize: '0.85rem',
      }}
    >
      {(params.value as { name: string; color: string }).name.toUpperCase().substring(0, 1)}
    </Avatar>
  );
}
