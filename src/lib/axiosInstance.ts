...
import { getClientSessionByCode, setClientSession } from "@/utils/session";
...
const getAuthToken = () => {
  if (typeof window !== "undefined") {
    const accessToken = getClientSessionByCode(AuthCookieNames.ACCESS_TOKEN);
    return accessToken;
  }
  return null;
};
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response.status === 401 && !originalRequest._retry) {
      ...
      const _refreshToken = getClientSessionByCode(AuthCookieNames.REFRESH_TOKEN);
